...
functions    sqrt cbrt abs floor ceil round trunc sin cos tan asin acos atan atan2
             ...
             pow(x,y) hypot(x,y) min(a,...) max(a,...) gcd lcm gamma mod sign clamp lerp fma copysign erf erfc beta logb nextafter ldexp dim signbit jn yn fact(n)
...
display      deg/rad, sci/fix, eng/std, prec <n>, base hex|dec|oct|bin, quiet, status, last
memory       ms, m+, m-, mr, mc
